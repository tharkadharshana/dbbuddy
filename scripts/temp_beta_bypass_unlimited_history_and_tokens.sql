...
-- 2. AI tokens: gives every known user a 25,000 ai_credits addon balance
--    (~USD 1 of AI usage, matching TOKEN_HARD_CAP; 1 addon unit = 1 unified
--    token, see billing.py get_user_subscription). This raises
--    tokens_total_available above tokens_limit, so check_ai_limit stops
--    blocking. It does NOT touch or reset tokens_used, and it stacks with
--    the existing plan tokens_limit rather than replacing it.
--
-- Idempotent: re-running does not duplicate history_limits (UPDATE only) but
-- WILL add another 25,000-credit addon pack per run for step 2 — run once.
...
-- ── 2. 25,000 ai_credits addon for every known user ─────────────────────────
INSERT INTO addon_purchases (user_email, addon_type, units_remaining)
SELECT email, 'ai_credits', 25000
FROM users;
