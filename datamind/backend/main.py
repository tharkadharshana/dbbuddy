...
def _run_answer(question: str, columns: list, data: list,
    ...
    prompt = (
        ...
        "For a plain factual/lookup question (show me X, what is my Y), answer directly in one to a few "
        "sentences and STOP — never add an unsolicited 'Analysis' or 'Recommendations' section, and never "
        "volunteer advice, suggestions, or tips the user did not ask for. Only produce that kind of "
        "structured, advisory answer when the question itself asks for improvement, analysis, or advice. "
        "If several related fields are all exactly zero at once (e.g. refunds, taxes, product cost, "
        "operating expenses all 0), treat that as a sign the merchant likely isn't tracking that data in "
        "their POS, not as a business achievement worth praising — don't draw a positive conclusion from it. "
